...
function p(stats: any) {
  ...
  process.stderr.write(
    `${ch.magentaBright(" -")} ${formatBytes(
      stats.deduplicated_size
    )}/${formatBytes(stats.compressed_size)}/${formatBytes(
      stats.original_size
    )} - ${stats.nfiles} files`
  )
}

export const stderr_progress = !is_tty
  ? undefined
  : function stderr_progress(err: any, key: number) {
      if (err.type === "progress_percent" && !err.finished) {
        process.stderr.clearLine(0)
        process.stderr.cursorTo(0)
        process.stderr.write(
          `${ch.magentaBright(" -")} ${formatBytes(err.current)}/${formatBytes(
            err.total
          )}`
        )
...
export function getTimestamp2() {
  ...
  return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}-${pad(
    d.getHours()
  )}h${pad(d.getMinutes())}m${pad(d.getSeconds())}s${d.getMilliseconds()}ms`
}
