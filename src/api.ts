...
function log_value<T, K extends keyof T>(opts: T, name: K) {
  if (opts[name]) {
    ...
    console.log(
      ` ${ch.bold.cyanBright("=")} ${ch.cyan(name)}: ${ch.magentaBright(
        Array.isArray(value)
          ? "\n  " +
              value
                .map((v: string) =>
                  v.replace(/:([^:]+)/, (_, a) => ":" + ch.yellowBright(a))
                )
                .join(",\n  ")
          : value
      )}`
...
export function get_archive_name(infos: RunBorgOnContainerOptions) {
  return (
    (infos.config.prefix || infos.infos?.Id || "chest") +
    "-" +
    helpers.getTimestamp()
  )
}

export function merge_labels(containers: ContainerInfo[]): {
  [label: string]: string
} {
  const res = {}
...
export function fill_defaults_from_container(
  infos: ContainerInspectInfo | ContainerInfo[],
  config: ChestConfig
) {
  const labels = Array.isArray(infos)
    ? merge_labels(infos)
    : infos.Config.Labels

  const backups_root_dir =
    process.env.CHEST_BACKUPS_DIR ??
    labels["chest.backups_root_dir"] ??
    config.backups_root_dir

  const backups_compose_dir =
    process.env.CHEST_BACKUPS_COMPOSE_DIR ??
    labels["chest.backups_compose_dir"] ??
    config.backups_compose_dir

  const passphrase =
    process.env.BORG_PASSPHRASE ??
    labels["chest.passphrase"] ??
    labels["borg.passphrase"] ??
    config.passphrase

  let prune =
    process.env.CHEST_PRUNE ??
    // ?? labels["borg.prune"]
    // ?? labels["chest.prune"]
    config.prune
  ...
  const repository =
    process.env.BORG_REPOSITORY ??
    labels["borg.repository"] ??
    labels["chest.repository"] ??
    path.join(
      config.backups_root_dir,
      /*labels["chest.name"] ??*/ labels["com.docker.compose.project"]
    )

  const prefix =
    process.env.CHEST_PREFIX ??
    labels["chest.prefix"] ??
    labels["com.docker.compose.service"]

  const archive =
    process.env.CHEST_ARCHIVE ?? `${prefix}-${helpers.getTimestamp()}`
...
export async function run_borg_backup(args: RunBorgOptions & Command) {
  ...
  if (args.passphrase) {
    ...
    if (!args.repository) {
      console.error(
        ch.yellowBright(" ! ") +
          "a passphrase was supplied but there is no borg repository"
      )
    }
  }
        ...
        if (
          ...
          if (data.type === "log_message") {
            if (
              ...
              data.msgid !== "Repository.AlreadyExists"
            ) {
...
export async function run_borg_backup_on_container(
  ...
  args.binds.push(
    ...args.infos.Mounts.map(
      (m) => `${m.Source}:${path.join(`/data`, m.Destination)}:rw`
    )
  )
  ...
  if (compose_config_files) {
    args.binds.push(
      ...compose_config_files
        .split(/,/g)
        .map((c) => `${c}:/data/${path.basename(c)}:ro`)
    )
  }

  const should_stop_and_restart =
    !args.keep_running && !!args.infos.State.Running
...
export async function do_restore(opts: DoRestoreOptions) {
  if (opts.infos.State.Running) {
    console.error(
      ch.redBright(
        "please shutdown the container and its whole stack before restoring to avoid inconsistencies and corrupted backups"
      )
    )
    return
  }
...
export async function do_backup(opts: DoBackupOptions) {
  ...
  if (is_ssh) {
    console.error(
      ch.bold.redBright(
        ` ⚠⚠⚠ warning : you are about to backup to a remote server. This is dangerous, you may be inadvertently trying to backup to an existing backup.\n`
      )
    )
  ...
  await run_borg_backup_on_container({
    ...opts,
    command: command_tag`
borg init --log-json -e ${opts.passphrase ? "repokey-blake2" : "none"} ${
      is_ssh ? opts.repository : "/repository"
    }
cd /data && borg create --progress --json --log-json --stats "::${
      opts.archive
    }" ./*
${
  prune
...
`,
    stdout(out, id) {
      console.log(
        STAR,
        `duration ${ch.greenBright(
          Math.round(100 * out.archive.duration) / 100
        )}s`
      )
      console.log(
        STAR,
        `deduplicated size ${ch.greenBright(
          helpers.formatBytes(out.archive.stats.deduplicated_size)
        )}, uncompressed ${ch.redBright(
          helpers.formatBytes(out.archive.stats.original_size)
        )}`
      )
      console.log(
        STAR,
        `repository size ${ch.greenBright(
          helpers.formatBytes(out.cache.stats.unique_csize)
        )}`
      )
...
//
export async function run_borg_backup_on_project(
  args: RunBorgOnProjectOptions & Command
) {
  args.binds ??= []
  ...
  const containers =
    ...
    (await docker.listContainers({
      label: [`com.docker.compose.project=${args.project_name}`],
    }))
  ...
  const infos = await Promise.all(
    containers.map((c) => new Container(docker.modem, c.Id).inspect())
  )
  ...
  if (args.services && services_map) {
    const srv = args.services
      .split(/,/g)
      .map((s) => s.split(/:/g).map((s) => s.trim()))
  ...
  for (let c of infos) {
    ...
    if (!working_dir) {
      ...
      if (working_dir) {
        args.binds.push(`${working_dir}:/data/__compose__`)
      }
    }
    ...
    args.binds.push(
      ...inspect.Mounts.filter(
        (m) =>
          !working_dir ||
          (path.relative(working_dir, m.Source).includes("..") &&
            working_dir !== m.Source)
      ).map((m) => {
        const res = `${m.Source}:${path.join(
          `/data/${service}`,
          m.Destination
        )}:rw`
        return res
  ...
  const deps_to_stop = figure_out_container_deps(infos).filter((d) => d.running)
  ...
  if (!args.keep_running) {
    ...
    await Promise.all(
      deps_to_stop
        ...
        .map((dep) => {
          console.log(
            ch.redBright(" ⏹︎ ") + "stopping " + ch.redBright([...dep.provides])
          )
  ...
  for (let c of deps_to_stop) {
    ...
    console.log(
      ch.greenBright(" ▶ ") + "restarting " + ch.greenBright([...c.provides])
    )
  ...
  try {
    do {
      ...
      if (infos.State.Running) {
        console.log(
          ch.redBright(" ⏹︎ ") + " stopping " + ch.redBright(infos.Name)
        )
...
export async function container_start(container: Docker.Container) {
  ...
  if (!infos.State.Running) {
    console.log(
      ch.greenBright(" ▶ ") + " starting " + ch.greenBright(infos.Name)
    )
    container.start()
  }
}
...
async function terminate() {
  ...
  for (let c of containers_to_restart) {
    try {
      ...
      containers_to_restart.delete(c)
    }
  }
  for (let c of containers_to_stop_and_delete) {
    try {
      ...
      containers_to_stop_and_delete.delete(c)
    }
  }
}
